...
controllers.controller('BoardWriteCtrl', [
  ...
  function ($scope, $window, $location, $http, $timeout, Upload, Alert, Board, initData) {
    ...
    boardId = write.boardId; // 리스트로 가기 위한
    ...
    $scope.onFileSelect = function (files) {
      ...
      uploadLength = $scope.write.files.length;
    ...
    $window.onbeforeunload = function () {
      if ($scope.beforeUnload) {
        return '이 페이지를 벗어나시면 작성 중인 글이 지워집니다.';
      }
    ...
    $scope.$on('$locationChangeStart', function (event) { // , next, current
      if ($scope.beforeUnload
        && !$window.confirm('이 페이지를 벗어나시면 작성 중인 글이 지워집니다.\n\n이 페이지를 벗어나시겠습니까?')) {
        event.preventDefault();
      }
    });
  }
...
controllers.controller('BoardViewCtrl', [
  '$scope', '$sce', '$location', '$anchorScroll', '$timeout', 'Alert', 'Board', 'initData',
